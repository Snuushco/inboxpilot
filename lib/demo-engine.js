...
function buildSummary(message, triage) {
  if (message.category === 'sales_opportunity') {
    return `${message.company} vraagt een voorstel voor ${message.entities.join(', ')}. Waarde is potentieel hoog en InboxPilot markeert dit als ${triage.bucket.toLowerCase()} met duidelijke deadline: ${message.deadlineText}.`;
  }
  ...
  if (message.category === 'service_request') {
    return `${message.company} kan nu niet doorwerken omdat een badge of toegang blokkeert. InboxPilot plaatst dit bovenaan vanwege hoge impact, korte SLA en risico op frustratie bij de klant.`;
  }
  ...
  return `Lage waarde of promotionele mail. InboxPilot bundelt dit in de digest en haalt het uit de primaire focus.`;
}
...
function generateDemo(planKey = 'team') {
  ...
  return {
    ...
    headline: top
      ? `InboxPilot zet ${top.subject.toLowerCase()} bovenaan en haalt ${noiseCount} ruis-mail${noiseCount === 1 ? '' : 's'} uit beeld.`
      : 'InboxPilot heeft geen demo-items gevonden.',
    stats: {
