...
const ACTIVATION_ENGINE_PATH = path.join(__dirname, '..', 'q557-inboxpilot-activation-engine.json');
...
async function sendViaResend(apiKey, { from, to, subject, html }) {
  ...
  const fromName = (from || '').match(/^([^<]*)</)?.[1]?.trim() || 'InboxPilot';
  const resendFrom = `${fromName} <inboxpilot@${resendDomain}>`;
...
function generateEmailHtml(lead, day) {
  ...
  const quickstartUrl = `https://inboxpilot.praesidion.com/app?leadId=${encodeURIComponent(lead.leadId || '')}`;
  ...
  switch (day) {
    ...
    case 27:
      ...
      break;
    default:
      bodyContent = `<p>Hoi ${firstName},</p><p>Dit is een update over je InboxPilot ${planLabel}-trial.</p>`;
  }
...
function getDripSubjectFallback(planKey, day) {
  const subjects = {
    0: `Je InboxPilot ${PLAN_LABELS[planKey]} staat klaar`,
  ...
  return subjects[day] || `InboxPilot update — dag ${day}`;
}
...
function buildDay0Body(firstName, company, planLabel, price, confirmation, quickstartUrl, endDate) {
  ...
  return `
    ...
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
      Je InboxPilot <strong>${planLabel}</strong>-omgeving voor ${company} is geactiveerd. 
  ...
  return `
    ...
    <div style="margin:20px 0;">
      <div style="background:#f9fafb;border-radius:6px;padding:16px;margin-bottom:12px;">
        ...
        <p style="margin:6px 0 0 0;color:#6b7280;font-size:14px;">InboxPilot heeft je inbox gesorteerd op urgentie. Kijk of de volgorde klopt.</p>
      </div>
      <div style="background:#f9fafb;border-radius:6px;padding:16px;margin-bottom:12px;">
...
function buildDay3Body(firstName, planLabel, company, sequenceInfo) {
  return `
    ...
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
      Hoi ${firstName}, na drie dagen begint InboxPilot ${planLabel} de structuur in je mailbox van ${company} te herkennen.
    </p>
    
    <div style="background:#f0f7ff;border-radius:8px;padding:20px;margin:20px 0;">
      <p style="color:#1e3a5f;font-weight:600;margin:0 0 8px 0;">Wat InboxPilot nu ziet:</p>
...
function buildDay7Body(firstName, planLabel, company, sequenceInfo) {
  return `
    ...
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
      Hoi ${firstName}, je eerste volledige week met InboxPilot ${planLabel} zit erop. 
  ...
  return `
    ...
    <div style="background:#fffbeb;border-radius:8px;padding:20px;margin:20px 0;">
      ...
      <p style="color:#374151;font-size:15px;margin:0;line-height:1.6;">
        Kijk naar de mails die InboxPilot als P2 of P3 labelt. 
...
function buildDay27Body(firstName, planLabel, price, endDate, checkout, sequenceInfo) {
  return `
    ...
    <div style="background:#f0f7ff;border-radius:8px;padding:20px;margin:20px 0;">
      ...
      <ul style="margin:0;padding:0 0 0 20px;color:#374151;line-height:1.8;">
        <li>Als InboxPilot je al rust, snelheid of grip oplevert: <strong>je hoeft niets te doen</strong>. 
  ...
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    ...
    <div style="background:#f9fafb;border-radius:0 0 8px 8px;padding:20px 32px;border:1px solid #e5e7eb;border-top:none;">
      <p style="color:#9ca3af;font-size:12px;margin:0;line-height:1.5;text-align:center;">
        ...
        Je ontvangt deze mail omdat je een InboxPilot trial hebt gestart.
      </p>
    </div>
  </div>
</body>
