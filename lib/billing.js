/**
 * InboxPilot MVP — Stripe Billing Module
 * 
...
function triggerBillingEmail(leadId, emailType) {
  ...
  switch (emailType) {
    case 'trial_ending':
      ...
      subject = `Je InboxPilot ${planLabel} trial eindigt over 3 dagen`;
      bodyHtml = buildTrialEndingEmail(state, planLabel, price);
      break;

    case 'payment_failed':
      subject = `⚠️ Betaling niet gelukt — InboxPilot ${planLabel}`;
      bodyHtml = buildPaymentFailedEmail(state, planLabel, price);
      break;

    case 'subscription_canceled':
      subject = `Je InboxPilot ${planLabel} is stopgezet`;
      bodyHtml = buildSubscriptionCanceledEmail(state, planLabel);
      break;

    case 'month2_welcome':
      subject = `Welkom bij maand 2 van InboxPilot ${planLabel} 🎉`;
      bodyHtml = buildMonth2WelcomeEmail(state, planLabel, price);
      break;

    case 'payment_recovered':
      subject = `✅ Betaling gelukt — InboxPilot ${planLabel} hersteld`;
...
function buildTrialEndingEmail(state, planLabel, price) {
  ...
  return `
    ...
    <p style="color:#6b7280;font-size:14px;line-height:1.5;">
      Als InboxPilot je rust, snelheid of grip oplevert — hoef je niets te veranderen.
    </p>
  `;
}
...
function buildPaymentFailedEmail(state, planLabel, price) {
  ...
  return `
    ...
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
      We konden je betaling van €${price} voor InboxPilot ${planLabel} niet verwerken${failCount > 1 ? ` (poging ${failCount})` : ''}.
    </p>
...
function buildSubscriptionCanceledEmail(state, planLabel) {
  return `
    <h2 style="color:#1e3a5f;margin:0 0 16px 0;font-size:22px;">Je InboxPilot ${planLabel} is stopgezet</h2>
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
    ...
    <p style="color:#6b7280;font-size:14px;line-height:1.5;">
      Bedankt dat je InboxPilot hebt geprobeerd. We hopen je in de toekomst weer te zien.
    </p>
  `;
}
...
function buildMonth2WelcomeEmail(state, planLabel, price) {
  return `
    ...
    <div style="background:#ecfdf5;border-radius:8px;padding:20px;margin:20px 0;">
      ...
      <ul style="margin:0;padding:0 0 0 20px;color:#374151;line-height:2;">
        <li>InboxPilot kent je mailpatronen nu beter — verwacht scherpere prioriteiten.</li>
...
function buildPaymentRecoveredEmail(state, planLabel) {
  return `
    ...
    <p style="color:#374151;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
      Je betaling is succesvol verwerkt. Je InboxPilot ${planLabel}-workspace is weer volledig actief.
    </p>
    <div style="background:#ecfdf5;border-radius:8px;padding:20px;margin:20px 0;">
      <ul style="margin:0;padding:0 0 0 20px;color:#374151;line-height:2;">
